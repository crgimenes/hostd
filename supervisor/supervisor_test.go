...
func newHarness(t *testing.T) *harness {
	...
	return &harness{t: t, dirs: dirs, buffer: newStore(t, root)}
}
...
// newStore opens a log store inside the test's own directory tree, so a test
// cannot reach a real host's logs however carelessly it is later edited.
func newStore(t *testing.T, root string) *logs.Store {
	t.Helper()
	...
	store, err := logs.Open(context.Background(), filepath.Join(dir, "logs.db"), logs.Options{})
	if err != nil {
...
func TestStartsAndCapturesOutput(t *testing.T) {
	...
	h.waitFor("stdout and stderr to be captured", func() bool {
		out := h.search(logs.Query{Service: "api", Stream: logs.StreamOut})
		errs := h.search(logs.Query{Service: "api", Stream: logs.StreamErr})
...
func TestEventsShareTheTimelineWithOutput(t *testing.T) {
	...
	records := h.search(logs.Query{Service: "api"})
	var sawOutput bool
...
func TestStartStopAreDeclarative(t *testing.T) {
	...
	if again != first {
		t.Fatalf("repeating start replaced the process: %d then %d", first, again)
	}
}
...
func TestOutputWrittenWhileAwayIsCaptured(t *testing.T) {
	...
	h.buffer = newStore(h.t, filepath.Dir(h.dirs.Spool))
	h.start(shell("api", `echo first; sleep 0.2; echo while-away; sleep 30`))
	defer h.stop()
...
func TestDeathWhileAwayIsNoticed(t *testing.T) {
	...
	// The durable record is the event: by now the loop may already have
	// started the service again, which clears the transient error field.
	missed := h.search(logs.Query{Kind: logs.EventMissed})
	if len(missed) != 1 {
		t.Fatalf("a death during the gap was not noticed:\n%s", h.dumpLog())
	}
	if missed[0].Service != "api" {
		t.Fatalf("the event names %q", missed[0].Service)
	}
}
...
func TestPlanMarksImpact(t *testing.T) {
	...
	gone := Destructive(changes)
	if len(gone) != 1 || gone[0].Service != "gone" {
		t.Errorf("Destructive returned %#v", gone)
	}
}
...
// What -debug reports has to be the shape of the durations, not an average
// that hides the tick that ran long.
func TestStatsReportsTickPercentiles(t *testing.T) {
	h := newHarness(t)
	sup := New(h.dirs, h.buffer)
	for i := 1; i <= 100; i++ {
		sup.sample(time.Duration(i) * time.Millisecond)
	}

	st := sup.Stats()
	if st.Ticks != 100 {
		t.Fatalf("counted %d ticks, expected 100", st.Ticks)
	}
	if st.TickP50 != 50*time.Millisecond {
		t.Fatalf("p50 is %s, expected 50ms", st.TickP50)
	}
	if st.TickP95 != 95*time.Millisecond {
		t.Fatalf("p95 is %s, expected 95ms", st.TickP95)
	}
	if st.TickMax != 100*time.Millisecond {
		t.Fatalf("max is %s, expected 100ms", st.TickMax)
	}
}

// The ring is a ceiling like any other list here: it keeps the last samples
// and never grows.
func TestStatsKeepsABoundedWindow(t *testing.T) {
	h := newHarness(t)
	sup := New(h.dirs, h.buffer)
	for range tickSamples * 3 {
		sup.sample(time.Millisecond)
	}
	st := sup.Stats()
	if st.Ticks != tickSamples*3 {
		t.Fatalf("counted %d ticks, expected %d", st.Ticks, tickSamples*3)
	}
