...
func TestLoadMissingFileUsesDefaults(t *testing.T) {
	...
	if cfg.LogBuffer != DefaultLogBuffer {
		t.Fatalf("log buffer = %d, want the default %d", cfg.LogBuffer, DefaultLogBuffer)
	}
}
...
func TestLoadReadsConfiguration(t *testing.T) {
	...
	err := os.WriteFile(path, []byte(`(host (tuple "log-buffer" 250))`), 0o600)
	if err != nil {
	...
	if cfg.LogBuffer != 250 {
		t.Fatalf("log buffer = %d, want 250", cfg.LogBuffer)
	}
}
...
func TestLoadRejectsBrokenConfiguration(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "hostd.filo")
	err := os.WriteFile(path, []byte(`(host (tuple "log-buffer"`), 0o600)
	if err != nil {
		t.Fatalf("write: %v", err)
	}
